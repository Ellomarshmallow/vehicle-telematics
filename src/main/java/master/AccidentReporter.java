...
public class AccidentReporter {
    public static SingleOutputStreamOperator detectAccident(
        ...
        return output
                ...
                // We gather the data that we need from each car that stopped, in this case
                // vid,highway,segment,direction and position.
                // We use a countwindow with 4 as is the number of tuples we want and set the other
                // parameter to 1 since one car can report more than one accident.
                .keyBy(
                        new KeySelector<
                                VehicleReport,
    ...
    // In this method the cars that have stopped more or 4 times will be selected included in the
    // output results
    private static class AccidentReportWindow
        ...
        public void apply(
            ...
            while (it.hasNext()) {
                ...
                // When the car has been stopped 4 times or more we gather the data and include it
                // in the output
                if (numEvents >= 4) {
